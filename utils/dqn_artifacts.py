...
<head>
  ...
  <title>DQN Policy Rollout</title>
  <style>
    :root {
      color-scheme: light;
      --wall: #2b2b2b;
...
<body>
  <main>
    <h1>DQN greedy policy rollout</h1>

    <section class="viewer">
      <svg id="rollout-svg" role="img" aria-label="DQN greedy policy rollout"></svg>
      <div class="controls">
  ...
  <script>
      ...
      for (let col = 0; col < data.n_cols; col += 1) {
        for (let row = 0; row < data.n_rows; row += 1) {
          ...
          svg.appendChild(svgElement("rect", {
            ...
            width: 1,
            height: 1,
            fill: cellColors[code] || cellColors[0],
            stroke: "none"
          }));
        }
      }
    ...
    const end = plotPoint(maxStep);
    const endA = svgElement("line", {
      x1: end[0] - 0.16,
      y1: end[1] - 0.16,
      x2: end[0] + 0.16,
      y2: end[1] + 0.16,
    ...
    svg.appendChild(endA);
    svg.appendChild(endB);
    ...
    const headingTip = svgElement("circle", {
      r: "0.07",
      fill: "#d98400"
    });
